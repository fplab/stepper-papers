...
live functional programming environment adopted for use in an upper-level
programming languages course taught by a co-author, where we teach exactly this style of
...
\code{fac(3)} already requires over a dozen small steps when every
...
hides all steps \code{debug eval(\$e)} except those involving a
function application by adding \code{debug stop(fac(\$v))}, the
trace collapses to just the key recursive calls on the \code{fac}
function and the final result
(Figure~\ref{fig:intro-fac-filtered}).

% TODO: rephrase the paragraph above to link into the image and explain how we did this.
...
\begin{itemize}
  ...
  \item We deploy the stepper in a university programming languages
    course and report quantitative usage data and survey responses from student assignments.
\end{itemize}
